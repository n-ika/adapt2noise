...
def plot_dec_boundary(all_points,decision):
    """
    ...
    plt.xlabel("VOT (ms)",fontsize=40)
    plt.ylabel("f0 (Hz)",fontsize=40)
    plt.legend(labels=["b","p"])
    plt.xticks(ticks=np.arange(-30,31,10),labels=[5*x for x in range(1,8)],fontsize=20)
    plt.yticks(ticks=np.arange(-30,31,10),labels=[20*x+200 for x in range(0,7)],fontsize=20)
    plt.savefig("./figs/single_dec_enc.jpg")

    return()

def plot_recon(stims_train,test_03,test_30,test_33,test_36,test_63,preds_03,preds_30,preds_33,preds_36,preds_63):
    """
    ...
    plt.savefig("./figs/recon_enc.jpg")

    return()

def plot_recon_dec(vae):
    """
    ...
    preds_36 = recon_data(vae,test_36)
    preds_63 = recon_data(vae,test_63)
    preds_30 = recon_data(vae,test_30)
    preds_03 = recon_data(vae,test_03)
    preds_33 = recon_data(vae,test_33)

    plot_recon(stims_train,test_03,test_30,test_33,test_36,test_63,preds_03,preds_30,preds_33,preds_36,preds_63)
    ...
    decision = dec_data(vae,all_points)
    
    plot_dec_boundary(all_points,decision)
