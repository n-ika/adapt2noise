...
path_d = os.path.abspath(root+'/data/')
...
data_csv = pd.read_csv(os.path.abspath(path_d+'/'+CORPUS+'_vowels_norm.csv'))
df_step = pd.read_csv(os.path.abspath(path_d+'/norm_test_wu.csv'))
data_csv = data_csv[(data_csv.f1<=1200.0) & (data_csv.duration>=0.03)]
stims_train = np.array(data_csv[['norm_duration','norm_f1']])
min_dur = np.min(stims_train[:,0])
max_dur = np.max(stims_train[:,0])
min_f1 = np.min(stims_train[:,1])
max_f1 = np.max(stims_train[:,1])
dur_step = df_step['z_step_dur_'+CORPUS][0]
f1_step = df_step['z_step_f1_'+CORPUS][0]


res = {'dur':[],'f1':[],'dec':[],'subject':[]}
durs = np.arange(min_dur,max_dur,0.1)
f1s = np.arange(min_f1,max_f1,0.1)
...
metadata.to_csv(os.path.abspath(path_a+'/'+ID+'_'+NUM+'_test.csv'))
